...
export const ConverseWithAI: React.FC<ConverseWithAIProps> = ({ autoPlay }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-2 sm:px-4 py-3 sm:py-6 flex flex-col h-[calc(100dvh-5.5rem)] sm:h-[calc(100vh-5rem)]">
      {/* Top Configuration Bar */}
      <div className="bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-slate-200/80 mb-3 sm:mb-4 flex-shrink-0">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-2.5 sm:gap-4">
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-2 w-full justify-between sm:justify-start">
            ...
            <button
              ...
              className="mt-4 p-2 sm:p-2.5 rounded-full border border-slate-200 hover:bg-slate-100 text-slate-600 transition-colors shadow-xs flex-shrink-0"
            >
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-2 justify-end w-full sm:w-auto">
            <button
              onClick={togglePlaybackSpeed}
              title="Change voice speed (1x, 0.75x, 0.5x)"
              className="flex items-center space-x-1 px-2.5 sm:px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs font-mono font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors border border-slate-200 shadow-2xs"
            >
              <FastForward className="w-3.5 h-3.5 text-indigo-600 flex-shrink-0" />
            ...
            <button
              ...
              className="flex items-center space-x-1 px-2.5 sm:px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs font-medium text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors border border-slate-200 shadow-2xs disabled:opacity-50"
            >
              <Download className="w-3.5 h-3.5 text-indigo-600 flex-shrink-0" />
            ...
            <button
              ...
              className="flex items-center space-x-1 px-2.5 sm:px-3 py-1.5 sm:py-2 text-[11px] sm:text-xs font-medium text-slate-600 hover:text-red-600 hover:bg-red-50 rounded-xl transition-colors border border-slate-200 hover:border-red-200 shadow-2xs"
            >
      ...
      <div className="flex-1 bg-slate-50/70 border border-slate-200 rounded-2xl p-3 sm:p-6 overflow-y-auto space-y-3 sm:space-y-4 mb-3 sm:mb-4 shadow-inner">
        {messages.map((msg) => {
          if (msg.sender === 'user') {
            return (
              <div key={msg.id} className="flex flex-col items-end">
                <div className="flex items-start space-x-1.5 sm:space-x-2 max-w-[92%] sm:max-w-[75%] flex-row-reverse space-x-reverse">
                  <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-slate-800 text-white flex items-center justify-center text-xs font-semibold shadow-xs flex-shrink-0 mt-1">
                    <User className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  </div>

                  <div className="bg-indigo-600 text-white rounded-2xl rounded-tr-xs p-3 sm:px-4 sm:py-3 shadow-sm">
                    <p className="text-xs sm:text-sm font-medium leading-relaxed">{msg.userOriginalText}</p>
            ...
            // AI Message
            return (
              <div key={msg.id} className="flex items-start space-x-2 sm:space-x-3 max-w-[95%] sm:max-w-[80%]">
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-indigo-600 text-white flex items-center justify-center text-xs font-semibold shadow-md flex-shrink-0 mt-1">
                  <Bot className="w-4 h-4 sm:w-4.5 sm:h-4.5" />
                </div>

                <div className="bg-white border border-slate-200/90 rounded-2xl rounded-tl-xs p-3 sm:p-4 shadow-xs space-y-2 sm:space-y-2.5 w-full">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                    ...
                    <div className="flex items-center space-x-1.5 flex-wrap sm:flex-nowrap flex-shrink-0 pt-1">
                      {msg.aiResponseTargetLang && (
                        <button
                          onClick={() =>
                            handlePlayMessageAudio(
                          ...
                          className={`flex items-center space-x-1 px-2 sm:px-2.5 py-1 rounded-lg text-[11px] sm:text-xs font-medium transition-all ${
                            playingMsgId === msg.id
                      ...
                      {msg.aiResponseTargetLang && (
                        <button
                          onClick={togglePlaybackSpeed}
                          className="px-1.5 sm:px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-[11px] sm:text-xs font-mono border border-slate-200/80 transition-colors flex items-center space-x-1"
                          title="Voice playback speed (1x, 0.75x, 0.5x)"
                        >
                      ...
                      {msg.aiResponseTargetLang && (
                        <button
                          ...
                          className="p-1 sm:p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors"
                          title="Copy text"
                        >
                          {copiedId === msg.id ? (
        ...
        >
          ...
          <input
            ...
            placeholder={
              isRecording
                ? `Recording voice...`
                : `Message AI (${aiLang.name})...`
            }
            disabled={isProcessing || isRecording}
            className="flex-1 min-w-0 bg-slate-50 border border-slate-200 rounded-xl px-3 sm:px-4 py-2 sm:py-2.5 text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:bg-white transition-all text-slate-800 placeholder-slate-400 disabled:opacity-60"
          />
          ...
          <button
            ...
            className="px-3.5 sm:px-5 py-2 sm:py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs rounded-xl shadow-md transition-all flex items-center space-x-1.5 disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
          >
