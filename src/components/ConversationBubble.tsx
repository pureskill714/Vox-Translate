  ...
  return (
    <div
      ...
      className={`flex flex-col my-3 transition-all ${
        isSpeakerA ? 'items-start pl-0 pr-4 sm:pr-12' : 'items-end pr-0 pl-4 sm:pl-12'
      }`}
    >
      {/* Header Badge */}
      <div className={`flex items-center space-x-2 mb-2 px-1 ${isSpeakerA ? 'flex-row' : 'flex-row-reverse space-x-reverse'}`}>
        <span
      ...
      {/* Main Card */}
      <div
        className={`w-full max-w-2xl rounded-2xl p-5 border shadow-sm relative overflow-hidden transition-all ${
          isSpeakerA
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              {item.pronunciationGuide && (
                <div className="mt-3 bg-amber-50/80 border border-amber-200/80 rounded-lg p-2 text-xs text-amber-900 flex items-center justify-between">
                  <div>
            ...
            {/* Audio Controls Bar */}
            <div className="pt-3 flex items-center justify-between bg-slate-100/70 -mx-5 -mb-5 p-3.5 px-5 border-t border-slate-200/60">
              <div className="flex items-center space-x-2">
                {/* Play/Pause Synthetic Voice */}
                <button
                  onClick={handlePlayAudio}
                  className={`flex items-center space-x-2 px-3.5 py-1.5 rounded-full font-medium text-xs transition-all shadow-xs ${
                    isPlaying
                ...
                >
                  ...
                  <span>{isPlaying ? 'Playing Voice...' : 'Play Voice'}</span>
                </button>

                {/* Playback speed toggle */}
                <button
                  onClick={togglePlaybackSpeed}
                  className="flex items-center space-x-1 px-2.5 py-1 bg-white hover:bg-slate-50 text-slate-700 rounded-lg text-[11px] font-mono border border-slate-200 shadow-xs"
